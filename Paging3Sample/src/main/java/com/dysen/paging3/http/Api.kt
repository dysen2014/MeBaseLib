...
//    fun deleteJson(url:String) = RxHttp.deleteJson(url).addHeader("token", token())
    ...
    suspend fun getProjects(url:String="https://api.github.com/search/repositories?sort=stars&q=Android&per_page=5&page=1") = get(url).toClass<Res.ProjListsRes>().await()
