...
object TimeUtil {
    ...
    var dateFormatMDofChinese1 = "MM月dd日 HH:mm"
     ...
     * @return
     */
    fun formatDataCurrent(dataFormat: String?): String? {
    ...
    fun date2TimeStamp(date_str: String?): String {
        try {
     ...
     */
    @Throws(ParseException::class)
    fun dateToStampYMDHM(s: String?): Long {
     ...
     */
    @Throws(ParseException::class)
    fun dateToStampYMD(s: String?): Long {
     ...
     *
     * @param date          日期时间
     * @param calendarField Calendar属性，对应offset的值， 如(Calendar.DATE,表示+offset天,
     * Calendar.HOUR_OF_DAY,表示＋offset小时)
     ...
     * @return
     */
    fun getTimeInterval(date: Date?): Array<Date?>? {
    ...
    fun getNextWeekFirstDay(): Calendar? {
        val cal = Calendar.getInstance()
     ...
     * @return
     */
    fun getNextMonday(gmtCreate: Date, num: Int): Array<Date?>? {
        val mondayPlus = getMondayPlus(gmtCreate)
        val currentDate = GregorianCalendar()
        val currentDate1 = GregorianCalendar()
        currentDate.add(GregorianCalendar.DATE, mondayPlus + num * 7)
        currentDate1.add(GregorianCalendar.DATE, mondayPlus + (num * 7 + 6))
        val monday = currentDate.time
        val sunday = currentDate1.time
        val dates = arrayOfNulls<Date>(2)
        dates[0] = monday
        dates[1] = sunday
        val stringByFormat = getStringByFormat(monday, dateFormatYMD)
        val stringByFormat1 = getStringByFormat(sunday, dateFormatYMD)


        //LogUtils.logi(stringByFormat + "," + stringByFormat1);
        return dates
    }


    // 获得当前日期与本周日相差的天数
    private fun getMondayPlus(gmtCreate: Date): Int {
        val cd = Calendar.getInstance()
        cd.time = gmtCreate
        // 获得今天是一周的第几天，星期日是第一天，星期二是第二天......
        val dayOfWeek = cd[Calendar.DAY_OF_WEEK] - 1 // 因为按中国礼拜一作为第一天所以这里减1
        //        System.out.println("===DAY_OF_WEEK==="+cd.get(Calendar.DAY_OF_WEEK));
